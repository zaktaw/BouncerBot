...
async function reportUser(msg, bot) {
    ...
    inboxChannel.send(`<@&${config.moderatorRoleID}> Report recieved from ${username} ${displayName ? ' (' + displayName + ')' : ''}: ${messageContent}`) // only show displayname if it is not identical to username
